...
# 视为「占位/未配置」的 SECRET_KEY 集合——命中则运行时随机生成，杜绝硬编码密钥漏洞
...
class Settings(BaseSettings):
    ...
    # ChromaDB 向量存储配置 (: 默认绝对路径，避免依赖启动 cwd)
    ...
    # ── ：运行期参数收口（集中可配可观测，消除魔法数字）──
    # 会话生命周期治理：容量上限 / 空闲 TTL / 历史轮数
    ...
    # ── ：生产环境（DEBUG=False）必须配置密钥，缺失即启动失败 ──
    # 开发环境（DEBUG=True）保留 的占位密钥自动随机化便利，不强制。
